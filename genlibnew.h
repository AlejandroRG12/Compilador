...
symrec *genconv(int typ, symrec *s)
	{
		symrec *s3;
		string *name = newtemp();
		s3 = putsym(name, typ & 0x1f | VAR_SPEC);
		switch(typ & 0x1f){
			case INTVAL:
				gencode(INT_IR, s, NULL, s3);
				break;
			case FVAL:
				gencode(FLOAT_IR, s, NULL, s3);
				break;
			case DOUBLEVAL:
				gencode(FLOAT_IR, s, NULL, s3);
				break;
			case CHARVAL:
				gencode(CHAR_IR, s, NULL, s3);
				break; 
		}
		return s3;
	}

	void genassign(code_ops ircode, VAR_DATA *sym1, VAR_DATA *sym3)
	{
		string *name;
		symrec *s, *s2, *s3;
		if(!isarray(sym1->typ)) {
			if((sym3->typ & 0x1f) == (sym1->typ & 0x1f))
				gencode(ircode, sym1->var, sym3->var, sym1->var);
			else {
				s3 = genconv(sym1->typ, sym3->var);
				gencode(ircode, sym1->var, s3, sym1->var);
			}
		} else {
			name = newtemp();
			s = putsym(name, sym1->typ & 0x1f | VAR_SPEC);
			name = newtemp();
			s2 = putsym(name, sym1->typ & 0x1f | VAR_SPEC);
			gencode(LOADA_IR, sym1->var, sym1->var2, s2);
			if((sym3->typ & 0x1f) == (sym1->typ & 0x1f)) {
				gencode(ircode, s2, sym3->var, s);
			} else {
				s3 = genconv(sym1->typ & (~ARRAY_SPEC), sym3->var);
				gencode(ircode, s2, s3, s);
			}
			gencode(STOREA_IR, s, sym1->var2, sym1->var);
		}
	}
